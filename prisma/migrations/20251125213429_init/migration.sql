...
CREATE TABLE `Habit_Fullfillment` (
    `habit_fulfillment_pk` INTEGER NOT NULL AUTO_INCREMENT,
    `habit_pk` INTEGER NOT NULL,
    `study_id` INTEGER NULL,
    `habit_fulfillment_year` INTEGER NOT NULL,
    `habit_fulfillment_week` INTEGER NOT NULL,
    `habit_fulfillment_day` INTEGER NOT NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NULL,

    PRIMARY KEY (`habit_fulfillment_pk`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `Emoji` (
    ...
    UNIQUE INDEX `Emoji_study_id_emoji_name_key`(`study_id`, `emoji_name`),
...
-- AddForeignKey
ALTER TABLE `Habit_Fullfillment` ADD CONSTRAINT `Habit_Fullfillment_habit_pk_fkey` FOREIGN KEY (`habit_pk`) REFERENCES `Habit`(`habit_pk`) ON DELETE CASCADE ON UPDATE CASCADE;
